...
public class CachedGuavaFieldConfigHelper implements FieldConfigHelper {
    ...
    public CachedGuavaFieldConfigHelper(FieldConfigHelper helper, int limit) {
        if (limit < 1) {
    ...
    boolean getFieldResult(AttributeType attributeType, String fieldName, Predicate<String> fn) {
        try {
            return resultCache.get(fieldName, () -> new CachedEntry(fieldName)).get(attributeType).getResultOrEvaluate(fn);
